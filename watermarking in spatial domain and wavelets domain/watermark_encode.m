...
Im = IMAGE;
for i = 1:length(WM.Tag);
    tag = dec2bin([int8(cell2mat(WM.Tag(i))),int8(':')],bin_len);
    value = dec2bin([int8(cell2mat(WM.Value(i))),int8(';')],bin_len);
    for j = 1:length(tag(:,1));
        for k = 1:bin_len;
            if mod(IMAGE(pos_shift+bin_len*(j-1)+k),2) ~= str2num(tag(j,k));
                if IMAGE(pos_shift+bin_len*(j-1)+k)<127
